...
export default function Create() {
  ...
  let addCategorise = (e) => {
    if (newcategorise && categories.includes(newcategorise)) {
      setNewcategorise("");
      return;
    }
      ...
      <div className="flex flex-wrap -mx-3 mb-6 space-y-3 mt-5">
        ...
        <div className="w-full px-3">
          ...
          <div className="flex items-center justify-center">
            <input
              value={newcategorise}
              onChange={(e) => setNewcategorise(e.target.value)}
              className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
              id="grid-password"
              type="text"
            ...
            <button type="button" onClick={addCategorise}>
              <svg
                ...
                strokeWidth={1.5}
                stroke="currentColor"
                className="size-6 bg-pirmary text-white  rounded-lg mb-3 ml-2"
              >
                <path
                  strokeLinecap="round"
          ...
          <div className="space-x-2">
            {categories.map((c) => (
              <span
                className="text-sm text-white bg-pirmary rounded-full px-2 py-1 "
                key={c}
              >
                {c}
              </span>
